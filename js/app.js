...
async function askNotif() {
  ...
  const tgl = el('tgl-notif'); if (tgl) tgl.checked = CFG.notif;
}
...
function updateExamChip() {
  const chip = el('exam-chip'); if (!chip) return;
...
function buildQuickStart() {
  ...
  const qs = el('quickstart'); if (!qs) return;
...
function buildSubjChips() {
  const chips = el('subj-chips'); if (!chips) return;
  const dow = new Date().getDay(), td = TIMETABLE[dow];
  ...
  chips.innerHTML = ordered.map(name => {
    ...
    return `<span onclick="document.getElementById('subj').value='${safeN}'" style="padding:5px 12px;background:${hi?'var(--acc-l)':'var(--s1)'};border:1px solid ${hi?'rgba(16,152,247,.2)':'var(--border)'};border-radius:20px;font-size:12px;font-weight:${hi?700:500};cursor:pointer;color:${hi?'var(--acc-t)':'var(--muted)'};display:inline-block;margin:2px 0;transition:opacity .15s">${safeN}</span>`;
  }).join('');
}
...
function openConfirm(title, body, okLabel, okStyle, cb) {
  ...
  const ov = el('confirm-ov'); if (ov) ov.classList.add('open');
}
...
function enterPhase(idx) {
  ...
  if (ph.t==='brk')  { showTimerScreen('break-screen'); renderBreakScreen(ph); tone('end'); startBreathe(); }
  else               { showTimerScreen('timer-screen'); renderTimerScreen(ph); tone('start'); startReminders(ph.d); scheduleCheckin(ph.d); requestWakeLock(); }
...
function nextBrk() {
  const e=el('brk-breathe'); if (e) e.textContent=BRK_MSGS[brkStep%4];
...
function recordSession() {
  ...
  STATS.hist.unshift({
    ...
    subj:   S.subject, method:S.type,
    mins:   focusMins, energy:S.energy, markCat:markCatSel
  });
...
function refreshStats() {
  ...
  const lbl=el('st-period-lbl');
  if (lbl) { const m={week:'This week',month:'This month',all:'All time'}; lbl.textContent=`${m[statPeriod]||'This week'} (${periodCount} session${periodCount!==1?'s':''})`; }
...
function _doRenderWeekChart() {
  ...
  data.forEach((d,i) => {
    ...
    if (d.today&&d.mins>0) {
      ...
      g.addColorStop(0,'#60c8fb'); g.addColorStop(1,'#1098f7');
      ctx.fillStyle=g;
...
function initSpecScroll() {
  const wrap=el('spec-scroll-wrap'); if (!wrap||wrap._scrollInited) return;
  wrap._scrollInited=true;
  wrap.addEventListener('wheel',e=>{if(Math.abs(e.deltaY)>Math.abs(e.deltaX)){e.preventDefault();wrap.scrollLeft+=e.deltaY*0.8;}},{passive:false});
...
function renderExamList() {
  ...
  wrap.innerHTML=upcoming.map(e=>{
    ...
    return `<div class="exam-row ${cls}">
      <div><div class="exam-name">${escHtml(e.subj)}</div><div class="exam-paper">${escHtml(e.paper||'')}${eDD&&eMM?`, ${eDD}/${eMM}`:''}</div></div>
      <div style="text-align:right"><div class="exam-days">${label}</div><div class="exam-days-lbl">${sub}</div></div>
    </div>`;
  }).join('');
}
...
function toggleAcc(h) {
  const b=h.nextElementSibling, a=h.querySelector('.acc-arrow'); if (!b) return;
...
function openSRReview() {
  ...
  srIdx=0;
  const ov=el('sr-ov'); if (ov) ov.classList.add('open');
  renderSRCard();
}
...
document.addEventListener('keydown', e => {
  ...
  const ts=el('timer-screen'), bs=el('break-screen'); if (!ts||!bs) return;
...
function injectManifest() {
  try {
    ...
    const m={name:'Solus',short_name:'Solus',start_url:'.',display:'standalone',background_color:'#0e1010',theme_color:'#0e1010',icons:[{src:icon,sizes:'192x192',type:'image/svg+xml'}]};
