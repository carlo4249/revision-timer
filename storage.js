...
function loadFromLocalStorage() {
  CFG        = lsGet(LS_KEYS.cfg,       DEFAULT_CFG);
  STATS      = lsGet(LS_KEYS.stats,     DEFAULT_STATS);
  SPACED     = lsGet(LS_KEYS.spaced,    []);
  TODAY_DONE = lsGet(LS_KEYS.todayDone, DEFAULT_TODAY_DONE);
  SPEC_CONF  = lsGet(LS_KEYS.spec,      {});
}

// -- Save a single key to localStorage (and queue Firestore sync) --
function save(key, value) {
  lsSet(key, value);
  if (currentUser && window.FIREBASE_ENABLED) {
    syncDocToFirestore();
  }
}
...
// -- Firestore: write all data as a single document --
let _syncTimer = null;
function syncDocToFirestore() {
  ...
  }, 1500); // debounce: batch rapid saves into one write
}
...
function clearAllData() {
  ...
  if (currentUser && window.firebaseDb) {
    window.firebaseDb.collection('users').doc(currentUser.uid).delete().catch(() => {});
  }
}
